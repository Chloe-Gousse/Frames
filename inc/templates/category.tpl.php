<!--<h1>Catégorie <?= $categoryName?></h1>--> <!-- $categoryName est récupéré grace à $categoryName = $_GET['catName']; -->
  ...
  <?php if (!empty($filteredArticles)): ?>
      <?php foreach ($filteredArticles as $currentArticleId=>$currentArticle) : ?>
     ...
     <?php endforeach ;?>
     <?php else : ?>
      <?= 'Il n\'y a pas d\'article disponible dans cette catégorie.'?>
